...
9. `petalinux-config -p qpix.linux/ -c kernel`. Once in the kernel menu, go to `Device Drivers/I2C support/I2C Hardware Bus support/Xilinx I2C Controller` (type `y` to select it). Save and exit.
10. `petalinux-build -p qpix.linux/ -c kernel -x finish`. This is needed because the kernel configuration was changed.
