...
7. From the `petalinux/config` directory copy the `system-user.dtsi` file to `qpix.linux/project-spec/meta-user/recipes-bsp/device-tree/files/` to enable the AXI IIC controller.
...
9. `petalinux-config -p qpix.linux/ -c kernel`. Once in the kernel menu, go to `Device Drivers/I2C support/I2C Hardware Bus support/Xilinx I2C Controller` (type `y` to select it). Also enable `Device Drivers/SPI support/User mode SPI device driver support`. Make sure `Cadence SPI controller` and `Xilinx SPI controller common module` are enabled, and `Xilinx Zynq QSPI controller` is disabled. Save and exit.
10. `petalinux-build -p qpix.linux/ -c kernel`. This is needed because the kernel configuration was changed.
11. `petalinux-build -p qpix.linux` will take many tens of minutes to build the first time.
12. `petalinux-package -p qpix.linux/ --boot --u-boot --fsbl qpix.linux/images/linux/zynq_fsbl.elf --fpga qpix.linux/images/linux/system.bit -o qpix.linux/images/linux/BOOT.bin` will package the output into a bootable image.
13. From the `qpix.linux/` directory, you can test the build by using `petalinux-boot --qemu --u-boot`. The image will boot virtually, and you can login with password `root` (change this in the config menu from step 5). You can test if simple command line utilities like `peek` were built into the image correctly by typing them at the prompt after logging in. CTRL+A, and then X, will exit a QEMU session.
...
| 0 | 0x43c00000 |  [17:16] | clk2_repl_en, clk_repl_en | enables replenishment clocks on opad_CLK. This clock is separate from the system clock. |
